...
use studio_application::{AppError, EditorSession};
use studio_core::state::AppState;
use studio_core::{Geo, Layer, LayerMode, Shape};
...
pub struct App {
    ...
    pub project: studio_application::ProjectService,
    /// Gecachte Projektliste; Dateisystemzugriff nur nach Projektänderungen.
    pub project_catalog: Vec<studio_core::ProjectInfo>,
    /// Gecachter Asset-Katalog; Metadaten werden nicht pro UI-Frame gelesen.
    pub asset_catalog: Vec<studio_core::AssetMeta>,
    ...
    pub ui_settings: studio_core::UiSettings,
    /// Periodischer Hub-Heartbeat läuft außerhalb des UI-Threads.
    hub_runtime: hub::HubRuntime,
    pub hub_status: crate::ui::HubTestStatus,
    pub hub_sync_error: Option<String>,
    /// Bestätigung vor unkoordinierter Verbindung bei ausgefallenem Hub.
    pub laser_uncoordinated_confirm: bool,
    pub laser_lease_force_confirm: Option<studio_application::HubLease>,
    ...
    /// Persistente, noch nicht automatisch angewandte Hub-Revisionen.
    pub project_inbox: Vec<studio_application::InboxEntry>,
    project_integration: project::ProjectIntegrationRuntime,
    pub project_integration_pending: bool,
    /// Geöffneter read-only Vergleich einer Hub-Revision.
...
impl App {
    pub fn new(
        ...
        // (praktisch fürs Testen: `studio datei.svg`).
        ...
        let hub_runtime = hub::HubRuntime::new(
            &ui_settings,
            &laser_backend.registry,
            material_service.library(),
        )?;
        let project_inbox = studio_application::list_inbox().unwrap_or_default();
        let project_integration = project::ProjectIntegrationRuntime::new()?;
        let project = studio_application::ProjectService::new();
        let project_catalog = project.list();
        if let Err(error) = studio_application::AssetService::cleanup_orphan_derived() {
            log::error!("Temporäre Crop-Assets bereinigen: {error}");
        }
        image::enrich_asset_tags_from_projects();
        let asset_catalog = studio_application::AssetService::list_visible()
            .unwrap_or_default()
            .into_iter()
            .collect();
        ...
        let mut app = Self {
            ...
            renderer,
            // Start-Ansicht per Env (Testhilfe): STUDIO_TAB=laser|preview.
            view: match std::env::var("STUDIO_TAB").as_deref() {
            ...
            ui_settings,
            hub_runtime,
            hub_status: crate::ui::HubTestStatus::Idle,
            hub_sync_error: None,
        ...
        if let Some(path) = auto_import {
            ...
            if std::env::var("STUDIO_FILL").is_ok() {
                app.toggle_fill();
            }
        }
            ...
            _ => {
                ...
                } = event
                {
                    ...
                    if pressed && !self.input_blocked() {
                        let trigger = studio_core::ShortcutTrigger::Mouse(
    ...
    fn fit_all(&mut self) {
        let b =
            studio_core::geometry::BBox::union_all(self.session.shapes.iter().map(|s| s.bbox()));
        if let Some(b) = b {
            self.canvas.cam.fit_bbox([b.x, b.y, b.w, b.h], 0.85);
        } else {
                            ...
                            && self
                                ...
                                .is_some_and(|layer| {
                                    layer.visible
                                        ...
                                        && layer.mode != studio_core::LayerMode::Image
                                })
                    })
                }),
        };
...
/// Ohne `STUDIO_FILL_STRESS` bleibt der normale Startzustand unverändert.
fn fill_stress_compounds_from_env() -> Option<usize> {
    std::env::var("STUDIO_FILL_STRESS")
        .ok()
...
mod tests {
    ...
    fn fill_stress_scene_erzeugt_unabhaengige_fill_shapes() {
        ...
        assert_eq!(state.layers[0].mode, studio_core::LayerMode::Fill);
    ...
    fn bild_import_legt_image_shape_an() {
        ...
        let dir = std::env::temp_dir().join("studio_img_test");
    ...
    fn text_wird_zu_pfad_shapes() {
        use studio_core::{text::text_to_contours, AppState, TextMeta};
    ...
    fn resize_schaukelt_nicht_auf() {
        use studio_core::{AppState, BBox, Geo, Handle};
        let mut s = AppState::new();
        s.add_shape(Geo::Rect {
            x: 0.0,
            y: 0.0,
        ...
        let target = studio_core::resize_to_cursor(start, Handle::Se, [150.0, 100.0]);
        for _ in 0..5 {
