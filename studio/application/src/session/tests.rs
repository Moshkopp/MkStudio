...
fn fertige_bezier_knoten_behalten_ihre_tangenten() {
    let nodes = vec![
        studio_core::bezier::BezierNode {
            p: (0.0, 0.0),
            h_in: Some((-5.0, 0.0)),
            h_out: Some((5.0, 0.0)),
        },
        studio_core::bezier::BezierNode::corner((20.0, 10.0)),
    ];
    let mut session = EditorSession::default();
...
fn valid_params() -> LayerParams {
    LayerParams {
        name: "Kontur".into(),
        mode: studio_core::LayerMode::Cut,
        speed_mm_s: 120.0,
        power_pct: 60.0,
        min_power_pct: 20.0,
        passes: 2,
...
fn layer_manager_uebernimmt_alle_zeilen_in_einem_undo_schritt() {
    ...
    state.add_shape(studio_core::Geo::Rect {
        x: 0.0,
        y: 0.0,
        w: 10.0,
        h: 10.0,
    ...
    drafts[0].mode = studio_core::LayerMode::Fill;
    ...
    assert_eq!(session.layers[0].mode, studio_core::LayerMode::Fill);
...
fn image_layer_kann_nicht_zu_vektor_umgewandelt_werden() {
    ...
    let before = session.layers[0].clone();
    let mut params = valid_params();
    params.mode = studio_core::LayerMode::Cut;
    let error = session.set_layer_params(0, params).unwrap_err();
...
fn vektor_layer_kann_nicht_versehentlich_image_werden() {
    ...
    session.state_mut_for_migration().dirty = false;
    let mut params = valid_params();
    params.mode = studio_core::LayerMode::Image;
    let error = session.set_layer_params(0, params).unwrap_err();
    assert_eq!(error.code(), "image_layer_mode");
    assert!(!session.dirty);
}
...
fn image_layer_darf_seine_bildparameter_aendern() {
    ...
    assert_eq!(session.layers[0].mode, studio_core::LayerMode::Image);
}
...
fn cut_layer_mit_altem_null_wert_bleibt_speicherbar() {
    ...
    session.set_layer_params(0, params).unwrap();
    assert_eq!(session.layers[0].mode, studio_core::LayerMode::Cut);
}
...
fn fill_layer_verlangt_positiven_zeilenabstand() {
    ...
    session.state_mut_for_migration().dirty = false;
    let mut params = valid_params();
    params.mode = studio_core::LayerMode::Fill;
    params.line_step_mm = 0.0;
    let error = session.set_layer_params(0, params).unwrap_err();
...
fn raster_layer_verlangt_positive_dpi() {
    ...
    session.state_mut_for_migration().dirty = false;
    let mut params = valid_params();
    params.mode = studio_core::LayerMode::Raster;
    params.dpi = 0.0;
    let error = session.set_layer_params(0, params).unwrap_err();
...
fn nan_werte_gelten_als_ungueltig_und_mutieren_nicht() {
    ...
    for (mut params, code) in [
        ...
        {
            let mut p = valid_params();
            p.mode = studio_core::LayerMode::Fill;
            p.line_step_mm = f64::NAN;
            (p, "line_step_invalid")
        },
        {
            let mut p = valid_params();
            p.mode = studio_core::LayerMode::Raster;
            p.dpi = f64::NAN;
            (p, "dpi_invalid")
        },
    ] {
...
fn bildparameter_setzen_ist_ein_undo_schritt() {
    use studio_core::{ImageMode, ImageParams};
    ...
    assert!(session.dirty);
    if let studio_core::Geo::Image { params: p, .. } = &session.shapes[0].geo {
...
fn job_preview_rastert_bild_assets() {
    ...
    assert!(preview
        .moves
        .iter()
        .any(|movement| movement.kind == studio_core::preview::MoveKind::Raster));
...
fn pattern_fill_weist_fehler_stabil_ab() {
    use studio_core::pattern_fill::{FillParams, Pattern};
...
fn import_test_square() -> String {
    ...
    img.write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
        .expect("PNG kodieren");
    studio_core::import_image(&studio_core::assets_dir(), &png, "quadrat.png")
        .expect("import_image")
        .id
}
...
fn trace_image_meldet_fehler_stabil() {
    ...
    let rect_idx = state.add_shape(studio_core::Geo::Rect {
        x: 0.0,
        y: 0.0,
        w: 5.0,
        h: 5.0,
