...
use studio_application::{BoxShape, EditorSession, PointPath};
...
pub struct PointerOutcome {
    ...
    pub error: Option<studio_application::AppError>,
}
...
impl CanvasState {
    ...
    pub fn hover_cursor(&self, session: &EditorSession) -> egui::CursorIcon {
        ...
        if let Some(bbox) = self.editable_selection_bbox(session) {
            ...
            for (handle, (x, y)) in studio_core::Handle::positions(&bbox) {
    ...
    fn editable_selection_bbox(&self, session: &EditorSession) -> Option<studio_core::BBox> {
            ...
            MouseButton::Left if pressed => {
                ...
                match self.tool {
                    ...
                    Tool::Bezier => {
                        ...
                        self.bezier_nodes
                            .push(studio_core::bezier::BezierNode::corner((w[0], w[1])));
    ...
    fn snapshot_selection(session: &EditorSession) -> Vec<(usize, studio_core::Shape)> {
        session
            .selected
            .iter()
    ...
    fn begin_node_edit(&mut self, session: &mut EditorSession, w: [f64; 2]) {
        ...
        for &shape_idx in session.selected.iter().rev() {
            ...
            for (node_idx, node) in nodes.iter().enumerate().rev() {
                for (part, point) in [
                    (studio_core::bezier::NodePart::HandleIn, node.h_in),
                    (studio_core::bezier::NodePart::HandleOut, node.h_out),
                    (studio_core::bezier::NodePart::Anchor, Some(node.p)),
                ] {
    ...
    pub fn on_cursor_move(&mut self, session: &mut EditorSession, new: [f32; 2]) {
        ...
        match &mut self.drag {
            ...
            Drag::EditNode { shape, node, part } => {
                ...
                let local = session.shapes.get(shape).map_or((w[0], w[1]), |shape| {
                    if shape.rotation.abs() > f64::EPSILON {
                        let pivot = shape.geo.bbox().center();
                        studio_core::geometry::rotate_point(
                            w[0],
                            w[1],
                            pivot.0,
                            pivot.1,
        ...
        match std::mem::replace(&mut self.drag, Drag::None) {
            Drag::Resize {
                ...
                let mut target = studio_core::resize_to_cursor(start_box, handle, w);
...
fn resize_cursor(handle: studio_core::Handle) -> egui::CursorIcon {
    use studio_core::Handle;
    match handle {
...
mod tests {
    ...
    fn verschobener_startknoten_blockiert_keine_weitere_bearbeitung() {
        ...
        assert!(matches!(
            ...
            Drag::EditNode {
                shape: 0,
                node: 0,
                part: studio_core::bezier::NodePart::Anchor
            }
        ));
    ...
    fn klick_nahe_start_schliesst_punktpfad_ohne_zusaetzlichen_knoten() {
        ...
        let studio_core::Geo::Polyline { pts, closed } = &session.shapes[0].geo else {
