//! Hub — optionaler lokaler Koordinationsdienst für Studio.
...
pub const NETWORK_OPT_IN_ENV: &str = "HUB_ALLOW_NETWORK";
...
impl ServerConfig {
    pub fn from_env() -> std::io::Result<Self> {
        let raw = std::env::var("HUB_BIND").unwrap_or_else(|_| DEFAULT_BIND.into());
    ...
    fn from_values(raw: &str, allow_network: bool) -> std::io::Result<Self> {
        let bind: SocketAddr = raw.parse().map_err(|error| {
            std::io::Error::new(
                ...
                format!("Ungültiges HUB_BIND '{raw}': {error}"),
            )
        })?;
        if !bind.ip().is_loopback() && !allow_network {
            return Err(std::io::Error::new(
                ...
                format!(
                    "HUB_BIND '{raw}' gibt Hub im Netzwerk frei. \
...
pub fn serve(config: ServerConfig) -> std::io::Result<()> {
    ...
    let state = Arc::new(SharedState {
        state: Mutex::new(ServerState::new(hub_data_dir())),
            ...
            Ok(mut stream) => {
                ...
                std::thread::spawn(move || {
                    if let Err(error) = serve_connection(&mut stream, &state) {
                        eprintln!("Hub-Verbindungsfehler: {error}");
                    }
                });
            }
            Err(error) => eprintln!("Hub-Accept-Fehler: {error}"),
        }
    }
    Ok(())
}
...
fn route(
    ...
    Ok(match (method, path) {
        ...
        ("GET", "/api/v1/assets") => json_body(
            &studio_core::list_assets(&state.data_dir.join("assets"))
        ...
        ("POST", "/api/v1/assets") => {
            ...
            match studio_core::store_asset(&state.data_dir.join("assets"), &transfer.meta, &bytes) {
                Ok(()) => json_body(
                    &studio_core::asset_meta(&state.data_dir.join("assets"), &transfer.meta.id)
                        .map_err(|error| std::io::Error::other(error.to_string()))?,
                )?,
                Err(_) => {
                    return Ok((
        ...
        ("GET", path) if path.starts_with("/api/v1/assets/") => {
            ...
            let meta = match studio_core::asset_meta(&store, &id.to_string()) {
                Ok(meta) => meta,
                Err(_) => return Ok(("404 Not Found", r#"{"error":"not_found"}"#.into())),
            };
            let bytes = studio_core::load_asset(&store, &id.to_string())
                .map_err(|error| std::io::Error::other(error.to_string()))?;
...
fn list_remote_revisions(
    ...
    for project in projects {
        ...
        for revision_dir in revision_dirs {
            ...
            let payload = std::fs::read_to_string(revision_dir.path().join("payload.laserproj"))?;
...
fn store_revision(
    ...
    let actual_hash = studio_core::assets::content_hash(upload.payload.as_bytes());
    ...
    let result = (|| {
        std::fs::write(temp_dir.join("payload.laserproj"), upload.payload)
...
fn store_catalog_change(
    ...
    if let Some(payload) = &upload.payload {
        if studio_core::assets::content_hash(payload.as_bytes()) != upload.content_hash {
        ...
        match upload.kind.as_str() {
            ...
            "material_profile" => {
                let profile: studio_core::MaterialProfile =
                    serde_json::from_str(payload).map_err(|_| StoreCatalogError::Invalid)?;
    ...
    } else if upload.content_hash != studio_core::assets::content_hash(b"") {
        return Err(StoreCatalogError::HashMismatch);
    }
...
fn store_workplace_backup(
    ...
    let actual_hash = studio_core::assets::content_hash(backup.payload.as_bytes());
...
fn hub_data_dir() -> PathBuf {
    std::env::var_os("HUB_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("local-data/hub"))
}
...
mod tests {
    ...
    fn gemeinsamer_katalog_erkennt_konflikt_und_behaelt_loeschung() {
        let dir =
            std::env::temp_dir().join(format!("hub-catalog-{}-{}", std::process::id(), now_unix()));
        let profile = studio_core::LaserProfile {
            id: "laser-a".into(),
            name: "Laser A".into(),
            ..Default::default()
        ...
        let hash = studio_core::assets::content_hash(payload.as_bytes());
    ...
    fn revision_wird_geprueft_atomar_und_idempotent_gespeichert() {
        ...
        let stored_payload = dir.join("projects/project-1/revisions/revision-1/payload.laserproj");
        ...
        assert!(
            store_workplace_backup(
                ...
                WorkplaceBackup {
                    ...
                    kind: "ui_settings".into(),
                    saved_at_unix: 44,
                    content_hash: studio_core::assets::content_hash(newer_payload.as_bytes()),
        ...
        assert!(
            store_workplace_backup(
                ...
                WorkplaceBackup {
                    ...
                    saved_at_unix: 43,
                    content_hash: studio_core::assets::content_hash(material_payload.as_bytes()),
