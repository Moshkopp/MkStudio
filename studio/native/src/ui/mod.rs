...
pub fn build(ui: &mut egui::Ui, app: &mut App) {
    ...
    // Offset-Werkzeug: Auswahl und Werkzeugreihenfolge sind austauschbar.
    // Sobald eine Kontur ausgewaehlt ist, sitzt die Bedienbox direkt an ihrer
    // Bounding-Box; die gruene gestrichelte Vorschau bleibt bis zum Haken rein
    // temporaer.
    if app.view == View::Design && app.canvas.tool == crate::tools::Tool::Offset {
        app.refresh_offset_preview();
        if let Some(bounds) = app.session.selection_bbox() {
            ...
            let mut commit = false;
            let mut cancel = false;
            egui::Area::new(egui::Id::new("offset_input"))
                .order(egui::Order::Foreground)
                .fixed_pos(pos)
                .show(ui.ctx(), |ui| {
                    ...
                    egui::Frame::new()
                        ...
                        .show(ui, |ui| {
                            if let Some(draft) = app.canvas.offset.as_mut() {
                                ui.spacing_mut().item_spacing = egui::vec2(6.0, 0.0);
                                ui.spacing_mut().interact_size.y = 32.0;
                                ui.style_mut().override_text_valign = Some(egui::Align::Center);
                                ui.horizontal_centered(|ui| {
                                    let direction = if draft.direction > 0.0 {
                                        "AUSSEN"
                                    } else {
                                        "INNEN"
                                    };
                                    let direction_icon =
                                        egui_material_icons::icons::ICON_SWAP_HORIZ
                                            .rich_text()
                                            .size(15.0);
                                    let direction_text = egui::RichText::new(direction).size(11.0);
                                    if ui
                                        .add_sized(
                                            [88.0, 32.0],
                                            egui::Button::new((direction_icon, direction_text))
                                                .gap(7.0)
                                                .fill(visuals.extreme_bg_color)
                                                .stroke(egui::Stroke::NONE)
                                                .corner_radius(8.0),
                                        )
                                        .on_hover_text("Richtung umkehren")
                                        .clicked()
                                    {
                                    ...
                                    if ui
                                        ...
                                        .on_hover_text("Offset übernehmen (Enter)")
                                        .clicked()
                                    {
                                        commit = true;
                                    }
